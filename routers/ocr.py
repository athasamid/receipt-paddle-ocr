...
async def predict_by_url(imageUrl: str, type: str = 'formated'): # type = formated | raw
    restfulModel = RestfulModel()
    ...
    if image_bytes.startswith(b"\xff\xd8\xff") or image_bytes.startswith(b"\x89PNG\r\n\x1a\n"):
        ...
        restfulModel = RestfulModel(resultcode=200, message="Success", data=result if type == "raw" else final, cls=OCRModel)
    else:
